...
@DisplayName("Cancel Travel Offering (X-API-Version: 1)")
class CancelTravelIntegrationTest extends IntegrationTest {
    ...
    void setUp() throws Exception {
        ...
        // Manager (owner)
        String managerEmail = "cancel-manager@example.com";
        ...
        String otherEmail = "other-cancel-manager@example.com";
        mockMvc.perform(post("/api/auth/register")
                .header(API_VERSION_HEADER, API_VERSION)
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
        ...
        // Admin
        String adminEmail = "admin-cancel@example.com";
        mockMvc.perform(post("/api/auth/register")
                .header(API_VERSION_HEADER, API_VERSION)
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
    ...
    @Test
    @DisplayName("Owner cancels a DRAFT travel returns 200 with CANCELLED status")
    void cancelTravel_draft() throws Exception {
        Long travelId = createDraftTravel(managerToken);

        mockMvc.perform(post("/api/travels/" + travelId + "/cancel")
                .header(API_VERSION_HEADER, API_VERSION)
    ...
    @Test
    @DisplayName("Owner cancels a PUBLISHED travel returns 200 with CANCELLED status")
    void cancelTravel_published() throws Exception {
        Long travelId = createDraftTravel(managerToken);
    ...
    @Test
    @DisplayName("Admin can cancel any travel")
    void cancelTravel_adminOverride() throws Exception {
        Long travelId = createDraftTravel(managerToken);
    ...
    void cancelTravel_nonOwnerRejected() throws Exception {
        Long travelId = createDraftTravel(managerToken);
    ...
    @Test
    @DisplayName("Cancelling a COMPLETED travel returns 409")
    void cancelTravel_completed() throws Exception {
        Long travelId = createDraftTravel(managerToken);
    ...
    @Test
    @DisplayName("Cancelling an already CANCELLED travel returns 409")
    void cancelTravel_alreadyCancelled() throws Exception {
        Long travelId = createDraftTravel(managerToken);
    ...
    void cancelTravel_notFound() throws Exception {
        mockMvc.perform(post("/api/travels/99999/cancel")
                .header(API_VERSION_HEADER, API_VERSION)
                .header("Authorization", "Bearer " + managerToken))
    ...
    void cancelTravel_unauthenticated() throws Exception {
        mockMvc.perform(post("/api/travels/1/cancel")
                .header(API_VERSION_HEADER, API_VERSION))
    ...
    void cancelTravel_travelerRejected() throws Exception {
        String travelerEmail = "traveler-cancel@example.com";
        String password = TestData.generateValidPassword();
