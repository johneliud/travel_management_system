...
@DisplayName("Publish Travel Offering (X-API-Version: 1)")
class PublishTravelIntegrationTest extends IntegrationTest {
    ...
    void setUp() throws Exception {
        ...
        // Manager (owner)
        String managerEmail = "publish-manager@example.com";
        ...
        String otherEmail = "other-publish-manager@example.com";
        mockMvc.perform(post("/api/auth/register")
                .header(API_VERSION_HEADER, API_VERSION)
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
        ...
        // Admin
        String adminEmail = "admin-publish@example.com";
        mockMvc.perform(post("/api/auth/register")
                .header(API_VERSION_HEADER, API_VERSION)
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
    ...
    @Test
    @DisplayName("Owner publishes a complete DRAFT travel returns 200 with PUBLISHED status")
    void publishTravel_success() throws Exception {
        Long travelId = createDraftTravel(managerToken);

        mockMvc.perform(post("/api/travels/" + travelId + "/publish")
                .header(API_VERSION_HEADER, API_VERSION)
    ...
    @Test
    @DisplayName("Admin can publish any travel")
    void publishTravel_adminOverride() throws Exception {
        Long travelId = createDraftTravel(managerToken);
    ...
    void publishTravel_nonOwnerRejected() throws Exception {
        Long travelId = createDraftTravel(managerToken);
    ...
    @Test
    @DisplayName("Publishing an already PUBLISHED travel returns 409")
    void publishTravel_alreadyPublished() throws Exception {
        Long travelId = createDraftTravel(managerToken);
    ...
    @Test
    @DisplayName("Publishing a CANCELLED travel returns 409")
    void publishTravel_cancelled() throws Exception {
        Long travelId = createDraftTravel(managerToken);
    ...
    @Test
    @DisplayName("Publishing a COMPLETED travel returns 409")
    void publishTravel_completed() throws Exception {
        Long travelId = createDraftTravel(managerToken);
    ...
    void publishTravel_notFound() throws Exception {
        mockMvc.perform(post("/api/travels/99999/publish")
                .header(API_VERSION_HEADER, API_VERSION)
                .header("Authorization", "Bearer " + managerToken))
    ...
    void publishTravel_unauthenticated() throws Exception {
        mockMvc.perform(post("/api/travels/1/publish")
                .header(API_VERSION_HEADER, API_VERSION))
    ...
    void publishTravel_travelerRejected() throws Exception {
        String travelerEmail = "traveler-publish@example.com";
        String password = TestData.generateValidPassword();
